...
<body>
    <div class="container">
        ...
        <button class="back" onclick="window.location.href='labtestdash.php'">Back</button>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if (empty($row['filePath']) && !empty($row['resultDetails'])) {
                            echo "<button class='back' onclick=\"viewTestResults('" . $row['id'] . "')\">View</button> ";
                        }
                        echo "<button class='accept' onclick=\"updateTestResult('" . $row['id'] . "')\">Update</button> ";
                        echo "<button class='reject' onclick=\"deleteTestResult('" . $row['id'] . "')\">Delete</button>";
                        echo "</td>";
